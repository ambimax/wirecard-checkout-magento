...
class Phoenix_WirecardCheckoutPage_Block_Seamless_CcMoto extends Mage_Payment_Block_Form_Cc
{
    protected function _construct()
    {
        parent::_construct();
        if (!Mage::getStoreConfigFlag('payment/wirecard_checkout_page_ccMoto/pci3DssSaqAEnable', Mage::app()->getStore()->getId()))
            $this->setTemplate('wirecard_checkout_page/seamless/ccmoto.phtml');
        else
            $this->setTemplate('wirecard_checkout_page/seamless/ccmoto-nopci.phtml');

    }

    public function showIssueDate()
    {
        return Mage::getStoreConfigFlag('payment/wirecard_checkout_page_ccMoto/show_issue_date', Mage::app()->getStore()->getId());
    }

    public function showIssueNumber()
    {
        return Mage::getStoreConfigFlag('payment/wirecard_checkout_page_ccMoto/show_issue_number', Mage::app()->getStore()->getId());
    }

	public function showCardholder()
	{
		return Mage::getStoreConfigFlag('payment/wirecard_checkout_page_ccMoto/show_cardholder', Mage::app()->getStore()->getId());
	}

	public function showCvc()
	{
		return Mage::getStoreConfigFlag('payment/wirecard_checkout_page_ccMoto/show_cvc', Mage::app()->getStore()->getId());
	}
